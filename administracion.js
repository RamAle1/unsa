...
app.controller('administracion-servicios-corpo', ['$scope', 'loginService', 'blockUI', '$timeout', '$window', '$filter','toastr', function ($scope, loginService, blockUI, $timeout, $window, $filter,toastr) {
	...
	$scope.variablesDeInicio = function () {
		...
		if ($scope.obtiensesion == undefined || $scope.obtiensesion == "" || $scope.obtiensesion == null) {
			...
			$window.location.href = 'http://www.testservices.tupatrimoniomexicano.com.mx/servicios/login.html'
	...
	$scope.createServiceAlumno = function () {
		...
		$timeout(function () {				
			...
			loginService.createMerge($scope.alumnoCrateMerge.id ,$scope.alumnoCrateMerge.clave,$scope.alumnoCrateMerge.carrera,$scope.alumnoCrateMerge.nombre,$scope.client.idServicio, $scope.client.servicio, $scope.client.costo).then(								
